...
n = int(input('후보자 수를 입력하시오 >>> '))

candidates = []
votes = {}

for i in range(n):
    people = input(f'{i+1}번째 후보자의 이름을 입력하시오 >>> ')
    candidates.append(people)
    votes[candidates[i]] = 0

allVotes = int(input('전체 투표 횟수를 입력하시오 >>> '))

for i in range(allVotes):
    num = int(input(f'{i+1}번째 투표 (1: 김일, 2: 김이, 3: 김삼) >>> '))
    if num == 1:
